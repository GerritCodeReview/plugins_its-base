...
configured through the rule bases in `etc/its/actions.config` (for global rules to be picked up by all configured ITS plugins) and
`etc/its/actions-@PLUGIN@.config` (for rules to be picked up only by @PLUGIN@) in the site directory. A rule base
is a git config file, and may contain an arbitrary number of
rules. Each rule can have an arbitrary number of conditions and
actions. A rule fires all associated actions, once all of its
conditions are met.
