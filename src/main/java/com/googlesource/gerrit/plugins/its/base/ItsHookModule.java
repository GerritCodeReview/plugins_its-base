...
public class ItsHookModule extends FactoryModule {
  ...
  protected void configure() {
    ...
    DynamicSet.bind(binder(), LifecycleListener.class)
        .to(Key.get(EventExecutor.class, Execution.class));
  ...
  int executionThreadPoolSize() {
    return pluginCfgFactory
        ...
        .getInt(EXECUTION_SECTION, KEY_THREAD_POOL_SIZE, DEFAULT_THREAD_POOL_SIZE);
  }

  @Provides
  @Singleton
  @Execution
  EventExecutor executionEventExecutor(WorkQueue workQueue, @ExecutionThreadPoolSize int poolSize) {
    return new EventExecutor(workQueue, poolSize, pluginName + "-execution");
  }

  @Provides
  @Singleton
  @Execution
  BoundedOrderedExecutor executionExecutor(@Execution EventExecutor executor) {
