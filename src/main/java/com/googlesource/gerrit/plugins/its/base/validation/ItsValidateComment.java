...
public class ItsValidateComment implements CommitValidationListener {
    ...
    switch (associationPolicy) {
      ...
      case SUGGESTED:
        ...
        if (issueIds.length > 0) {
          ...
          StringBuilder sb = new StringBuilder();
          sb.append("Add an issue-id matching ");
          sb.append(itsConfig.getIssuePattern().pattern());
          sb.append(" for an existing ticket in ");
          sb.append(pluginName);
          sb.append(" to the commit message");
          details = sb.toString();
  ...
  private List<CommitValidationMessage> commitValidationFailure(
    ...
    if (existenceCheck == ItsExistenceCheckResult.CONNECTIVITY_FAILURE) {
      ...
      boolean isMandatory = itsConfig.getItsAssociationPolicy() == ItsAssociationPolicy.MANDATORY;
      ret.add(
          new CommitValidationMessage(
              synopsis,
              isMandatory
                  ? CommitValidationMessage.Type.ERROR
