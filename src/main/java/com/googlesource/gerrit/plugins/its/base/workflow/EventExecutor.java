...
import com.googlesource.gerrit.plugins.its.base.workflow.ActionController.ChangeKey;
...
public class EventExecutor implements LifecycleListener {
  ...
  private final KeyLock<ChangeKey> keyLock = new KeyLock<>();
  ...
  public void execute(@Nullable ChangeKey orderingKey, Runnable task) {
    if (executor == null) {
      task.run();
      return;
    }
  ...
  private void submit(@Nullable ChangeKey orderingKey, Runnable task) {
    try {
  ...
  private final class QueuedTask implements Runnable {
    @Nullable private final ChangeKey orderingKey;
    private final Runnable task;

    QueuedTask(@Nullable ChangeKey orderingKey, Runnable task) {
