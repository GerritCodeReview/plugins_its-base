...
public class ActionExecutor {
  ...
  private void execute(ActionRequest actionRequest, Map<String, String> properties) {
    try {
      Action action = getAction(actionRequest.getName());
      if (action == null) {
        its.performAction(properties.get("issue"), actionRequest.getUnparsed());
      } else {
        action.execute(properties.get("issue"), actionRequest, properties);
      }
    } catch (IOException e) {
  ...
  public void execute(Collection<ActionRequest> actions, Map<String, String> properties) {
    for (ActionRequest actionRequest : actions) {
      execute(actionRequest, properties);
    }
  }
}
