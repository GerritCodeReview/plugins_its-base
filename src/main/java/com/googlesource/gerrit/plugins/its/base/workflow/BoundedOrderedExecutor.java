...
public class BoundedOrderedExecutor {
  ...
  @Inject
  BoundedOrderedExecutor(EventExecutor executor) {
  ...
  public void execute(@Nullable Object orderingKey, Runnable task) {
    ...
    if (admitted.needsRelease.getAndSet(true)) {
      admissions.release();
    }
  }
  ...
  private final class AdmittedTask implements Runnable {
    ...
    private final AtomicBoolean needsRelease = new AtomicBoolean();
    ...
    public void run() {
      try {
        ...
        if (needsRelease.getAndSet(true)) {
          admissions.release();
        }
      }
    }
