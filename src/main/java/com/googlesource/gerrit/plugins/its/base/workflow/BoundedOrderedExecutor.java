...
public class BoundedOrderedExecutor {
  ...
  public BoundedOrderedExecutor(EventExecutor executor) {
  ...
  public void execute(@Nullable Object orderingKey, Runnable task) {
    ...
    AdmittedTask admitted = new AdmittedTask(task);
    try {
      executor.execute(
    ...
    if (admitted.admissionReadyForRelease.getAndSet(true)) {
      admissions.release();
    }
  }
  ...
  private final class AdmittedTask implements Runnable {
    ...
    private final AtomicBoolean admissionReadyForRelease = new AtomicBoolean();
    ...
    public void run() {
      try {
        ...
        if (admissionReadyForRelease.getAndSet(true)) {
          admissions.release();
        }
      }
    }
