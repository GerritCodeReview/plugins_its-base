...
public final class TroubleClient {
  ...
  private static final String FORMAT_GERRIT_CORRECTION_TITLE_V2 = "Gerrit fix for %s (%s)";
  private static final String FORMAT_GERRIT_CORRECTION_TITLE_V1 = "Gerrit fix for %s";

  private static final String FORMAT_GERRIT_CORRECTION_DESCRIPTION = "Automatically generated by Gerrit.";
  ...
  private static class Correction {
     ...
     * Helper constructor.
     */
    private Correction(final String platform, final String targetBranch, final String username) {
      this.title = String.format(FORMAT_GERRIT_CORRECTION_TITLE_V2, platform, targetBranch);
      this.text = FORMAT_GERRIT_CORRECTION_DESCRIPTION;
      this.username = username;
    }
  }
   ...
   */
  public TroubleClient.Correction getCorrection(final String platform, final String targetBranch,
                                                final Boolean includePackages) throws IOException {
    StringBuilder url = new StringBuilder(String.format(FORMAT_GET_POST_CORRECTION, baseApiUrl, ticketId));
    String content = getRequest(url.toString(), apiUser, apiPass);

    TroubleClient.Correction correction = null;
    TroubleClient.Correction[] corrections = GSON.create().fromJson(content, TroubleClient.Correction[].class);

    // Look for new magic
    String magic = String.format(FORMAT_GERRIT_CORRECTION_TITLE_V2, platform, targetBranch);
    for (TroubleClient.Correction corr : corrections) {
      if (magic.equals(corr.title)) {
        correction = corr;
      }
    }

    // Backwards compatibiliy: old magic
    if (correction == null) {
      magic = String.format(FORMAT_GERRIT_CORRECTION_TITLE_V1, targetBranch);
      for (TroubleClient.Correction corr : corrections) {
        if (magic.equals(corr.title)) {
          correction = corr;
        }
    ...
    if (correction != null && includePackages) {
   ...
   * Returns all entries for the given project.
   */
  public void addFixToProject(final Integer projectId, final String platform, final String targetBranch) throws IOException {
    // find the entry
    ...
    TroubleClient.Correction correction = getCorrection(platform, targetBranch, true);
   ...
   */
  public void createOrUpdateFix(final String platform, final String targetBranch, final int packageId) throws IOException {

    // get the correction id
    TroubleClient.Correction correction = getCorrection(platform, targetBranch, false);
    if (correction == null) { // create correction info
      correction = new TroubleClient.Correction(platform, targetBranch, impersonatedUser);
      ...
      // update description to new format
      correction.title = String.format(FORMAT_GERRIT_CORRECTION_TITLE_V2, platform, targetBranch);
      correction.text = FORMAT_GERRIT_CORRECTION_DESCRIPTION;
    }
   ...
   * Delete a package in Trouble.
   */
  public void deletePackageFromFix(final String platform, final String targetBranch, final int packageId) throws IOException {
    TroubleClient.Correction correction = getCorrection(platform, targetBranch, false);
    if (correction == null) {
